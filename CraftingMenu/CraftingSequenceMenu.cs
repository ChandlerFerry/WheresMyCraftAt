...
public static class CraftingSequenceMenu
{
    ...
    public static List<CraftingStepInput> ConvertCoEData(string coeJson)
    {
        ...
        foreach (var config in coeSim.config)
        {
            ...
            {
                input.SuccessAction = config.actions.win?.ToLowerInvariant() == "end" ? SuccessAction.End :
                    config.actions.win?.ToLowerInvariant() == "step" ? SuccessAction.GoToStep : SuccessAction.Continue;

                input.SuccessActionStepIndex = config.actions.win_route.HasValue ? (int)config.actions.win_route.Value - 1 : 1;
                input.FailureAction = config.actions.fail?.ToLowerInvariant() == "step" ? FailureAction.GoToStep :
                    config.actions.fail?.ToLowerInvariant() == "loop" ? FailureAction.RepeatStep : FailureAction.Restart;

                input.FailureActionStepIndex = config.actions.fail_route.HasValue ? (int)config.actions.fail_route.Value - 1 : 1;
            }

            input.ConditionalGroups = new List<ConditionalGroup>();
            if (config.filters != null)
            {
                foreach (var filter in config.filters)
                {
                    ...
                    {
                        GroupType = filter.type?.ToLowerInvariant() == "or" ? ConditionGroup.OR :
                            filter.type?.ToLowerInvariant() == "not" ? ConditionGroup.NOT : ConditionGroup.AND,
